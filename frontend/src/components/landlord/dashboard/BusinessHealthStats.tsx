...
import { Bed, Home, Star, TrendingUp, Users } from "lucide-react"
   ...
   <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group">
    ...
    <div className="flex flex-col">
     ...
     <div className="flex items-end gap-2">
      <span className="text-3xl font-bold text-slate-900 dark:text-white mt-1">{stats.occupancyRate.toFixed(1)}%</span>
      <span className="text-sm text-slate-500 mb-1">rate</span>
     </div>
     <div className="mt-2 text-xs text-emerald-600 dark:text-emerald-400 font-medium flex items-center">
      {stats.occupiedSeats} / {stats.totalSeats} seats occupied
     </div>
    </div>
   </div>
    ...
    <div className="flex flex-col">
     ...
     <div className="mt-2 text-xs text-indigo-600 dark:text-indigo-400 font-medium flex items-center">
      {stats.longTermVacantSeats > 0 ? (
       <span>{stats.longTermVacantSeats} long-term vacant</span>
      ) : "High demand"}
     </div>
    </div>
   </div>
